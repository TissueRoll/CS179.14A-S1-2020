...
sf::VertexArray verts(sf::PrimitiveType::Lines, 0);

bool readFromAvailableText() {
    std::string input;
    std::ifstream settings("hw06_settings.txt");
    if (settings.is_open()) {
...
void initializeSettings() {
    if (readFromAvailableText()) {
        std::cout << "hw06_settings.txt successfully loaded.\n";
    } else {
        std::cout << "hw06_settings.txt not loaded. Using default values.\n";
...
void update(const sf::Time& elapsed) {
    ...
    verts.clear();
    verts.append(sf::Vertex(userBallEntity.ball.getPosition(), sf::Color::White));
    verts.append(sf::Vertex(userBallEntity.ball.getPosition() + userBallEntity.velocity, sf::Color::White));
    verts.append(sf::Vertex(userBallEntity.ball.getPosition(), sf::Color::Blue));
    verts.append(sf::Vertex(userBallEntity.ball.getPosition()+acceleration, sf::Color::Blue));
    for (int i = 0; i < num_circles; ++i) {
        verts.append(sf::Vertex(otherBallEntities[i].ball.getPosition(), sf::Color::White));
        verts.append(sf::Vertex(otherBallEntities[i].ball.getPosition() + otherBallEntities[i].velocity, sf::Color::White));
    }
}
...
int main () {
    srand(time(NULL));
    sf::RenderWindow window(sf::VideoMode(window_w, window_h), "HW 6");
